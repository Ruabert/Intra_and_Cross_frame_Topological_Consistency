...
class SDM_to_Mask(nn.Module):
    def __init__(self, k1=None, k2=None):
        self.k1 = k1
        self.k2 = k2
        super(SDM_to_Mask, self).__init__()

    def forward(self, x):
        x = torch.where(x>0, x-0.1, x)
        x = torch.relu(x)

        region = 2 * torch.sigmoid(x * 5000) - 1

        return region

class Mask_to_Edge(nn.Module):
    def __init__(self, max_k=3, min_k=3):
        super(Mask_to_Edge, self).__init__()
...
class SDM_to_Edge(nn.Module):
    def __init__(self, max_k=3, min_k=3):
        super(SDM_to_Edge, self).__init__()
        self.max_pooling_1 = nn.MaxPool2d(min_k, 1, min_k//2)
...
class UStructureEnhancedNetwork(nn.Module):
    def __init__(self, arg=None):
        super(UStructureEnhancedNetwork, self).__init__()

        self.arg_config = arg

        if arg is None:
            self.pipeline = UNetMSRF(in_ch=1,
                                     num_classes=3,
                                     base_ch=32,
                                     block='BasicBlock', pool=True)
        else:
            self.pipeline = UNetMSRF(in_ch=self.arg_config.in_chan,
                                     num_classes=self.arg_config.classes,
                                     base_ch=self.arg_config.base_chan,
                                     block=self.arg_config.block, pool=True)
    ...
    def forward(self, x_in):
        ...
        x_t_1, x_t = x_in[:, 1, ...],  x_in[:, 0, ...]  # x_t: bs, 1, h, w

        bs, c, h, w = x_t.shape

        # t frame into pipeline
        t_seg,  t_reg, _ = self.pipeline(x_t)

        # t-1 frame into pipeline
        t_1_seg, t_1_reg, _ = self.pipeline(x_t_1)
