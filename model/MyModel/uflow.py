...
from model.MyModel.unet_utils import inconv
...
class PWCFlow(nn.Module):
    ...
    def forward(self, feature_pyramid1, feature_pyramid2, actions=None):
        ...
        for level, (features1, features2) in reversed(
            ...
            if self._use_cost_volume:
                cost_volume = uflow_utils.compute_cost_volume(features1_normalized, warped2_normalized)
            else:
            ...
            cost_volume = F.leaky_relu(cost_volume, negative_slope=self._leaky_relu_alpha)
            ...
            if self._action_channels is not None and self._action_channels > 0:
                ...
                gx = gx.repeat(B, 1, 1, 1)
                gy = gy.repeat(B, 1, 1, 1)
    ...
    def _build_flow_layers(self):
        ...
        for i in range(0, self._num_levels):
            layers = nn.ModuleList()
            last_in_channels = (64+32 * self._channel_multiplier) if not self._use_cost_volume else (81+32 * self._channel_multiplier)
            if self._action_channels is not None and self._action_channels > 0:
                last_in_channels += self._action_channels + 2 # 2 for xy augmentation
            if i != self._num_levels-1:
                last_in_channels += 2 + self._num_context_up_channels * self._channel_multiplier
            ...
            layers.append(
                nn.Conv2d(
                    in_channels=block_layers[-1] * self._channel_multiplier,
                    out_channels=2,
                    kernel_size=(3, 3),
                    padding=1))
            if self._shared_flow_decoder:
    ...
    def _build_refinement_model(self):
        ...
        layers = []
        last_in_channels = 2 + 32 * self._channel_multiplier
...
class UFlow(nn.Module):
    def __init__(self, num_channels=3, num_levels=3, channel_multiplier=4, use_cost_volume=True, action_channels=0):
        super(UFlow, self).__init__()
        
        
        block = get_block('BasicBlock')
        self.inc = inconv(in_ch=num_channels, out_ch=32, block=block)

        self._pyramid = PWCFeaturePyramid(num_levels=num_levels, num_channels=32, channel_multiplier=channel_multiplier)
        self._flow_model = PWCFlow(num_levels=num_levels, num_channels_upsampled_context=32, channel_multiplier=channel_multiplier,
                                               use_cost_volume=use_cost_volume, use_feature_warp=True,
                                               action_channels=action_channels)
        
        self.outc = nn.Conv2d(2, 2 * num_channels, 1)
    def forward(self, img1, img2, actions=None):
        ...
        """
        # img1 and img2 scale to [0, 1]
        B, C, H, W = img1.shape[:]
        if img1.max() > 1.0 or img1.min() < .0:
            B, C, H, W = img1.shape[:]
            img1_norm = F.normalize(img1.view(B, C, -1).contiguous(), dim=-1)
            img1_norm = img1_norm.view(B, C, H, W).contiguous()

        if img2.max() > 1.0 or img2.min() < .0:
            B, C, H, W = img2.shape[:]
            img2_norm = F.normalize(img2.view(B, C, -1).contiguous(), dim=-1)
            img2_norm = img2_norm.view(B, C, H, W).contiguous()


        fp1 = self._pyramid(self.inc(img1))
        fp2 = self._pyramid(self.inc(img2))
