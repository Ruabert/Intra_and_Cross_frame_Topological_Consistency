...
def cosine_lr_scheduler_with_warmup(optimizer, init_lr, epoch, warmup_epoch, max_epoch, lr_min=1e-5, lr_warm=1e-6):
    if epoch >= 0 and epoch <= warmup_epoch and warmup_epoch != 0:
        lr = lr_warm
        if epoch == warmup_epoch:
            lr = init_lr
        ...
        lr = (lr_min + 0.5 * (init_lr - lr_min) * (
                    1.0 + math.cos((epoch - warmup_epoch) / (max_epoch - warmup_epoch) * math.pi)))
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
