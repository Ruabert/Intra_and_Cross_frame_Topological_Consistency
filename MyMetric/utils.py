...
from MyMetric import metrics
import numpy as np
import pdb
import medpy.metric as metric


def calculate_distance(label_pred, label_true, C, percentage=95):
    ...
    for i in range(C - 1):
        tmp_surface = metrics.compute_surface_distances(label_true == (i + 1), label_pred == (i + 1),
                                                        spacing_mm=[1, 1, 1])
...
def calculate_dice_split(pred, target, C, block_size=64 * 64 * 64):
    ...
    dice, intersection, summ = calculate_dice(pred, target, C)
